...
`gl(n, k, length = n*k, labels = seq_len(n), ordered = FALSE)` where `n` is the number of levels, `k` is the number of replications for each level.
```{r}
...
`interaction` gives us a factor which is the result of interactions of factors.
```{r}
f1 <- gl(2, 1, 8, labels = c("broken", "fixed"))
f2 <- gl(4, 2, 8, labels = c("spade", "shovel", "rake", "bin"))
interaction(f1,f2)
```
...
DFs are to matrices what lists are to vectors. Columns must be a single class, but each column can be any class. Note that recycling occurs to preserve dimensions. They have an attribute `$row.names` which can be set.
```{r}
df <- data.frame(id = 1:4, truth = c(T,T,F,T), d = matrix(21:24, nrow=2, ncol=2))
row.names(df) <- letters[5:8]
```

Data frames can be converted to a matrix with `data.matrix`, where values will be coerced so as to all be the same class.
```{r}
data.matrix(df)
```
# Control Structures {.tabset .tabset-fade .tabset-pills}

Note that we can assign from control structures:
```{r}
y <- if(3 > 2) {
  "yep"
} else {
...
## Loop Functions {.tabset .tabset-fade .tabset-pills}

The `*apply` family apply functions over a list or vector. See also [https://nsaunders.wordpress.com/2010/08/20/a-brief-introduction-to-apply-in-r/]. Much of what these functions do is covered by the `plyr` library.
...
We can automatically remove NAs that interfere with calculations by setting `na.rm = TRUE`. 
```{r}
...
`by` is a wrapper to `tapply` applied to data frames. Here we create linear models on the columns of iris, excluding the Species factor, which we set as the indices to subset on. Then we produce a `summary` for the linear model of one of the species.
```{r}
ourModels <- by(iris[, -5], iris$Species, lm)
summary(ourModels$setosa)
```
...
Where `sapply` tries to guess the correct format, `vapply` allows you to explicitly set it. This is useful when writing functions where you want to be very certain of what is being returned.
...
`...` argument indicates a variable number of arguments that are usually passed on to other functions. Here we extend `plot` and accept any arguments that it would take.
```{r}
myplot <- function(x, y, type="l", ... ) {
  plot(x, y, type = type, col="green", main="Green lines", ... )
}
myplot(1:10, 1:10+rnorm(10))
```
